...
const url =
  "mongodb+srv://hlev2454:passwordmongo@barknetcluster.ksy8pmw.mongodb.net/AuthDB?retryWrites=true&w=majority";
...
app.post("/New_Post", upload.single("picture"), async (req, res) => {
  const { username, picture, caption, hashtag, likesCount } = req.body;
  console.log(req.body);
...
require("./postDetails");
const display_post = mongoose.model("posts");
app.post =
  ("/Explore",
  async (req, res) => {
    try {
      const posts_d = await display_post.find();
      res.json(posts_d);
    } catch (error) {
